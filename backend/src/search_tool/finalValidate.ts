...
import { SearchAnswerSchema, SearchAnswer } from '../utils/schemas';
...
async function repairSearchAns(obj: any): Promise<{answer : string ; sources : string[]}>{
    ...
    const response = await model.invoke(
        [
            new SystemMessage(
                [
                    "You fix json objects to match the schema",
            ...
            new HumanMessage(
                [
                    "Make this exactly to a schema . Ensure sources is an array of URL strings",
                    "Input JSON:",
                    JSON.stringify(obj)
                ].join('\n')
            ),
